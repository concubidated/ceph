...
object_t _InodeStore::get_object_name(inodeno_t ino, frag_t fg, const char *suffix)
{
  char n[60];
...
void _InodeStore::encode_bare(bufferlist &bl, const bufferlist *snap_blob) const
{
  ::encode(inode, bl);
  if (is_symlink())
    ::encode(symlink, bl);
...
void _InodeStore::encode(bufferlist &bl, const bufferlist *snap_blob) const
{
  ENCODE_START(5, 4, bl);
...
void CInode::encode_store(bufferlist& bl)
{
  ...
  _InodeStore::encode(bl, &snap_blob);
}

void _InodeStore::decode_bare(bufferlist::iterator &bl,
...
void _InodeStore::decode(bufferlist::iterator &bl, bufferlist& snap_blob)
{
...
{
  bufferlist snap_blob;
  _InodeStore::decode(bl, snap_blob);
...
void _InodeStore::dump(Formatter *f) const
{

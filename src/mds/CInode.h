...
public:
  ...
  _InodeStore() : oldest_snap(CEPH_NOSNAP) { }
...
class InodeStore : public _InodeStore {
public:
  ...
  void encode_bare(bufferlist &bl) const {
    _InodeStore::encode_bare(bl, &snap_blob);
  }
  void decode_bare(bufferlist::iterator &bl) {
    _InodeStore::decode_bare(bl, snap_blob);
  }
  static void generate_test_instances(std::list<InodeStore*>& ls);
};

// cached inode wrapper
class CInode : public MDSCacheObject, public _InodeStore {
  /*
